...
export const generateTransactionReceipt = (transaction: Transaction) => {
  ...
  doc.text('HikmaCash', pageWidth / 2, pageHeight / 2, {
    align: 'center',
    angle: 45,
  });
  ...
  // Footer text
  const footerText = `Generated by HikmaCash on ${new Date().toLocaleString()}`;
  ...
  // Save the PDF
  const fileName = `HikmaCash-Receipt-${transaction.id}.pdf`;
  doc.save(fileName);
};