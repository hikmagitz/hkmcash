...
export const generateTransactionReceipt = (transaction: Transaction, enterpriseName?: string) => {
  // Always get the most current enterprise name from localStorage or context
  // This ensures we use the latest value even if the user just changed it
  const getCurrentEnterpriseName = (): string => {
    // First try to use the passed parameter (most current)
    if (enterpriseName?.trim()) {
      return enterpriseName.trim();
    }
    
    // Fallback to localStorage if no parameter passed
    try {
      const storedName = localStorage.getItem('enterpriseName');
      if (storedName?.trim()) {
        return storedName.trim();
      }
    } catch (error) {
      console.warn('Could not access localStorage for enterprise name');
    }
  ...
  // Get the current enterprise name - this will always be up-to-date
  const receiptTitle = getCurrentEnterpriseName();
  ...
  // Main title using the current enterprise name in header
  ...
  // Footer text with current enterprise name branding
  ...
  // Watermark with current enterprise name
  ...
  // Save the PDF with current enterprise name in filename
