...
export const generateTransactionReceipt = (transaction: Transaction, enterpriseName: string) => {
  ...
  // Use "luxfoods" as the main title for the receipt
  const receiptTitle = 'luxfoods';
  // Use the provided enterprise name for company details or fallback
  const companyName = enterpriseName?.trim() || 'luxfoods';
  ...
  // Header with luxfoods branding
  doc.setFillColor(...primaryColor);
  doc.rect(0, 0, pageWidth, 50, 'F');
  
  // Main title "luxfoods" in header
  ...
  // Transaction details in a clean table format (removed category)
  const details = [
  ...
  // Footer text with luxfoods branding
  ...
  // Save the PDF with luxfoods in filename
